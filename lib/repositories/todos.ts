...
import type { CreateTodoInput, Todo, TodoFilters, UpdateTodoInput } from "@/lib/schemas/todo";
...
const todoColumns = "id, text, done, priority, category, due_date, created_at, completed_at";
...
export async function listTodos(userId: number, filters: TodoFilters): Promise<Todo[]> {
...
export async function createTodo(userId: number, input: CreateTodoInput): Promise<Todo> {
    const result = await getPool().query<TodoRow>(
...
export async function updateTodo(
    ...
    if (patch.done !== undefined) {
        ...
        assignments.push(`completed_at = case when $${values.length} then now() else null end`);
    }
