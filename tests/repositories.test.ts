...
import { createTodo, deleteTodo, getTodoStats, listTodos, updateTodo } from "@/lib/repositories/todos";
...
describe("todos repository", () => {
    ...
    it("marks a todo as done and sets completedAt", async () => {
        const userId = await createTestUser("alice@example.com");
        const createdTodo = await createTodo(userId, { text: "Task", priority: "low", category: "other" });
        ...
        const reopenedTodo = await updateTodo(userId, createdTodo.id, { done: false });
    ...
    it("never leaks todos between users", async () => {
        ...
        const aliceTodo = await createTodo(aliceId, { text: "Alice task", priority: "low", category: "other" });
        await createTodo(bobId, { text: "Bob task", priority: "low", category: "other" });
    ...
    it("filters by category, priority, status and search", async () => {
        const userId = await createTestUser("alice@example.com");
        await createTodo(userId, { text: "Buy groceries", priority: "medium", category: "shopping" });
        await createTodo(userId, { text: "Review pull requests", priority: "high", category: "work" });
        const doneTodo = await createTodo(userId, { text: "Call mom", priority: "low", category: "personal" });
        await updateTodo(userId, doneTodo.id, { done: true });

        const workTodos = await listTodos(userId, { ...defaultFilters, category: "work" });
        expect(workTodos.map((todo) => todo.text)).toEqual(["Review pull requests"]);

        const highPriorityTodos = await listTodos(userId, { ...defaultFilters, priority: "high" });
        expect(highPriorityTodos).toHaveLength(1);

        const completedTodos = await listTodos(userId, { ...defaultFilters, status: "completed" });
        expect(completedTodos.map((todo) => todo.text)).toEqual(["Call mom"]);

        const searchedTodos = await listTodos(userId, { ...defaultFilters, search: "groc" });
        expect(searchedTodos.map((todo) => todo.text)).toEqual(["Buy groceries"]);
    });

    it("treats like wildcards in search as literals", async () => {
        const userId = await createTestUser("alice@example.com");
        await createTodo(userId, { text: "Reach 100% coverage", priority: "low", category: "work" });
        await createTodo(userId, { text: "Reach 100x speedup", priority: "low", category: "work" });

        const searchedTodos = await listTodos(userId, { ...defaultFilters, search: "100%" });
        expect(searchedTodos.map((todo) => todo.text)).toEqual(["Reach 100% coverage"]);
    });

    it("respects limit and offset ordering by newest first", async () => {
        const userId = await createTestUser("alice@example.com");
        for (const label of ["first", "second", "third"]) {
            await createTodo(userId, { text: label, priority: "low", category: "other" });
        }

        const firstPage = await listTodos(userId, { ...defaultFilters, limit: 2, offset: 0 });
        expect(firstPage.map((todo) => todo.text)).toEqual(["third", "second"]);

        const secondPage = await listTodos(userId, { ...defaultFilters, limit: 2, offset: 2 });
        expect(secondPage.map((todo) => todo.text)).toEqual(["first"]);
    });

    it("computes stats in a single query", async () => {
        const userId = await createTestUser("alice@example.com");
        await createTodo(userId, { text: "Open task", priority: "low", category: "other" });
        await createTodo(userId, { text: "Overdue task", priority: "low", category: "other", dueDate: "2020-01-01" });
        const doneTodo = await createTodo(userId, { text: "Done task", priority: "low", category: "other" });
        await updateTodo(userId, doneTodo.id, { done: true });
