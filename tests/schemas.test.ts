...
import { createTodoSchema, todoFiltersSchema, todoIdSchema, updateTodoSchema } from "@/lib/schemas/todo";

describe("createTodoSchema", () => {
    it("accepts a minimal valid todo and fills defaults", () => {
        const result = createTodoSchema.parse({ text: "Buy milk" });
        expect(result).toEqual({ text: "Buy milk", priority: "medium", category: "other" });
    });
    ...
    it("rejects text over 500 characters", () => {
        expect(createTodoSchema.safeParse({ text: "x".repeat(501) }).success).toBe(false);
    });

    it("rejects an unknown priority", () => {
        const result = createTodoSchema.safeParse({ text: "x", priority: "SUPER-HACKER" });
        expect(result.success).toBe(false);
    });

    it("rejects a malformed due date", () => {
        expect(createTodoSchema.safeParse({ text: "x", dueDate: "tomorrow" }).success).toBe(false);
    });
});
...
describe("todoFiltersSchema", () => {
    ...
    it("rejects an unknown status", () => {
        expect(todoFiltersSchema.safeParse({ status: "everything" }).success).toBe(false);
    });
});
...
describe("credentialsSchema", () => {
    it("accepts a valid email and password", () => {
        const result = credentialsSchema.parse({ email: "user@example.com", password: "longenough" });
        expect(result.email).toBe("user@example.com");
    });

    it("rejects an invalid email", () => {
        expect(credentialsSchema.safeParse({ email: "nonsense", password: "longenough" }).success).toBe(false);
    });
